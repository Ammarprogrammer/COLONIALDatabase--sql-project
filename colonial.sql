...
CREATE VIEW NHTrips_VW AS
SELECT TripID, TripName, StartLocation, Distance, MaxGrpSize, Type, Season
FROM Trip
WHERE State = 'NH';

CREATE VIEW ReservationCustomer_VW AS
SELECT R.ReservationID, R.TripID, T.TripName, R.TripDate, C.CustomerNum, 
       C.LastName, C.FirstName, C.Phone
FROM Reservation R
INNER JOIN Customer C ON R.CustomerNum = C.CustomerNum
INNER JOIN Trip T ON R.TripID = T.TripID
INNER JOIN TripGuides TG ON T.TripID = TG.TripID
INNER JOIN Guide G ON TG.GuideNum = G.GuideNum
WHERE G.FirstName = 'Glory' AND G.LastName = 'Unser'
   OR G.FirstName = 'Susan' AND G.LastName = 'Kiley';
